...
$(document).on("click", ".Button_Plus", function () {
  ...
  jQuery.ajax({
    // ? url: "@Url.Action("Transaction_Controller_Middle_Create_Update_Alternative", "Transaction")",
    url: "https://localhost:44381/Transaction/Transaction_Controller_Middle_Create_Update_Alternative",
    type: "POST",
...
$(document).on("click", ".Button_Delete_Supply", function () {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      jQuery.ajax({
        // ? url: "@Url.Action("Transaction_Controller_Middle_Delete_Alternative", "Transaction")",
        url: "https://localhost:44381/Transaction/Transaction_Controller_Middle_Delete_Alternative",
        type: "POST",
        data: { ID_Insumo: ID_Insumo },
        success: function (data) {
          if (data.result) {
